...
export interface WorkboxEvents {
  ...
  'update:error': {
     ...
     */
    err: pRetry.FailedAttemptError | undefined;
  };
 ...
 */
type WorkboxEmitteryApi = Pick<Emittery<WorkboxEvents>, SelectedEmitteryMethods>;
    ...
    try {
      updateServiceWorker = registrationFn({
        ...
        onOfflineReady: () => {
          ...
          if (/safari/gi.test(userAgent.browser.name ?? '')) {
